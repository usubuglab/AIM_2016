...
GISattrbIN$ProjHydr_WS=GISattrbIN$Hydr_WS*(GISattrbIN$QExProbC/100)
GISattrbIN$ProjHydr_S=GISattrbIN$Hydr_S*(GISattrbIN$QExProbPt/100)
...
#wetland and HerbW (18): #wetland or beaver: MS-LS-3002 XS-RM-6073 MN-SS-1106 MS-LS-3027 MN-LS-1014;#wet but sampled: XE-ss-5129  XN-LS-4002 XN-SS-4116#subset(GISattrbIN,select=c(SiteCode,PerHerbWS,PerWoodyWS,PerWaterS),subset=SiteCode %in% c('MS-LS-3002','XS-RM-6073','MN-SS-1106' ,'MS-LS-3027','MN-LS-1014','XE-ss-5129'  ,'XN-LS-4002' ,'XN-SS-4116')) #subset(GISattrbIN,select=c(SiteCode,Reason1,PerHerbWS,PerWoodyWS,PerWaterS),subset=PerHerbWS>0)
#subset(GISattrbIN,select=c(SiteCode,Reason1,QExProbC,QExProbPt,Hydr_S,Hydr_WS,ProjHydr_WS,ProjHydr_S),subset=substr(Reason1,1,1)=="D")
#nrow(subset(GISattrbIN,select=c(SiteCode,Reason1,IntDensS,IntDensC),subset=IntDensC>0.4 & substr(Reason1,1,1)=="D"))
