...
FQCY_VEG=setNames(merge(nFQCY_VEG,FQCY_VEG,by="UID"),c("UID","nINVASAQ_CHECK","nINVASH_CHECK","nINVASW_CHECK","nNATIVW_CHECK","nSEGRUSH_CHECK","INVASAQ_CHECK","INVASH_CHECK","INVASW_CHECK","NATIVW_CHECK","SEGRUSH_CHECK"))                  
...
#convert to percent
for (i in 12:ncol(pvtnonnative)){
  pvtnonnative[,i]=round(pvtnonnative[,i]/FQCY_VEG$nNATIVW_CHECK*100,digits=0)
}

pvtnonnative$NativeWoodyIssue=ifelse(pvtnonnative$NativeWoody<pvtnonnative$INVASW_CHECK,1,0)
pvtnonnative$NonNativeHerbIssue=ifelse(pvtnonnative$NonNativeHerb<pvtnonnative$INVASH_CHECK,1,0)
pvtnonnative$NonNativeAquaticIssue=ifelse(pvtnonnative$NonNativeAquatic<pvtnonnative$INVASAQ_CHECK,1,0)
pvtnonnative$NonNativeWoodyIssue=ifelse(pvtnonnative$NonNativeWoody<pvtnonnative$INVASW_CHECK,1,0)
pvtnonnativeissues=subset(pvtnonnative,NativeWoodyIssue==1|NonNativeHerbIssue==1|NonNativeAquaticIssue==1|NonNativeWoodyIssue==1)
write.csv(pvtnonnativeissues,'pvtnnonnativeissues_all.csv')
