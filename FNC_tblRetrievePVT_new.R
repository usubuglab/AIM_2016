...
ColCheck = function(TBL,VAR){
  ...
  if(length(MissingCheck)>0){
    for (c in 1: length(MissingCheck)){
      ...
      else if(MissingCheck[c]=='IND'){
        #this needs to retrieve the max index number
        TBL$NEW=seq(from=unlist(IndMax),to=unlist(IndMax)+nrow(TBL)-1)
        IndMax=max(TBL$NEW)+1
        assign('IndMax',IndMax, envir = .GlobalEnv)}
      else{
        TBL$NEW=NA
      }
...
XwalkParam=sqlQuery(wrsa1314,sprintf("select SAMPLE_TYPE,PARAMETER,Name_Xwalk,Table_Xwalk,Type_Xwalk,Parameter_Xwalk,Notes, case when right(SAMPLE_TYPE,1)='X' then left(SAMPLE_TYPE,len(SAMPLE_TYPE)-1) else SAMPLE_TYPE end as 'TABLE' from tblXWALK where Name_XWALK='%s' and ACTIVE='TRUE'",XwalkName))
...
# 
# Xwalk = function(
#   Source='SQL',XwalkName='WRSA',XwalkDirection='',COL='',
#   Table='',ALL='N',Filter='',UIDS='BLANK',SiteCodes='',Dates='',
#   Years='',Projects='',Protocols='',Parameters='',ALLp='N'
# ) {
#   if(Source=='SQL' | Table==''){
#     TBLtmp = tblRetrieve(
#       Table=Table,ALL=ALL,Filter=Filter,UIDS=UIDS,SiteCodes=SiteCodes,
#       Dates=Dates,Years=Years,Projects=Projects,Protocols=Protocols,
#       Parameters=Parameters,ALLp=ALLp
#     )
#     #XwalkDirection=''#user given control in the function, rather than assuming based on source
#   }
#   else if (Source=='R') {
...
#   TBLtmp=ColCheck(TBLtmp,c(VAR,'PARAMETER','TRANSECT','POINT','RESULT',COL)) 
#   
#   XwalkParam = sqlQuery(wrsa1314,
#     sprintf(
#       "select SAMPLE_TYPE,PARAMETER,Name_Xwalk,Table_Xwalk,Type_Xwalk,Parameter_Xwalk,Notes,
#       case
#       when right(SAMPLE_TYPE,1)='X' 
#       then left(SAMPLE_TYPE,len(SAMPLE_TYPE)-1)
#       else SAMPLE_TYPE 
#       end as 'TABLE'
#       from tblXWALK
#       where Name_XWALK='%s' and ACTIVE='TRUE'",
#       XwalkName
#     )
#   )
#   
#   XwalkTBL = sqldf::sqldf(
#     sprintf(
#       "select  UID, TRANSECT, POINT,upper(XwalkParam.Table_Xwalk) as 'TABLE',
#       (
#       case 
#       when '%s'='_Xwalk' or  XwalkParam.Type_Xwalk='' or  XwalkParam.Type_Xwalk is null 
#       then XwalkParam.SAMPLE_TYPE 
#       else XwalkParam.Type_Xwalk 
#       end
#       ) as SAMPLE_TYPE,
#       (
#       case 
#       when '%s'='_Xwalk' or  XwalkParam.PARAMETER_Xwalk='' or  XwalkParam.PARAMETER_Xwalk is null 
#       then XwalkParam.PARAMETER 
#       else XwalkParam.PARAMETER_Xwalk 
#       end
#       ) as PARAMETER,
#       RESULT,FLAG,IND,ACTIVE,OPERATION,INSERTION,DEPRECATION,REASON %s
#       from TBLtmp
#       JOIN XwalkParam 
#       on upper(TBLtmp.PARAMETER)= upper(XwalkParam.PARAMETER%s) 
#       and upper(TBLtmp.SAMPLE_TYPE)= upper(XwalkParam.[TABLE%s])
#       ",
#       XwalkDirection,
#       XwalkDirection,
#       gsub("'", "]", ifelse(COL=='','', gsub(", '", ",[", paste(", " , as.sql.in(COL), sep='') ) ) ),
#       XwalkDirection,
#       ifelse(XwalkName=='FMstr', '', XwalkDirection)
#     )
#   )
#   
#   XwalkTBL$SAMPLE_TYPE = ifelse(
#     toupper( substr( XwalkTBL$SAMPLE_TYPE, nchar(XwalkTBL$SAMPLE_TYPE), nchar(XwalkTBL$SAMPLE_TYPE) ) ) == 'X',
#     substr( XwalkTBL$SAMPLE_TYPE, 1, nchar(XwalkTBL$SAMPLE_TYPE)-1 ),
#     XwalkTBL$SAMPLE_TYPE
#   )
#   
#   names(XwalkParam) = toupper( names(XwalkParam) )
#   
#   if( nrow(TBLtmp) != nrow(XwalkTBL) ) {
#     cat( sprintf('\nWARNING! Different number of rows in original (%s rows) and xwalk (%s rows) tables.',
#                  nrow(TBLtmp),
#                  nrow(XwalkTBL)),
#          sep = ""
#     )
#     XwalkParamCheck = subset( XwalkParam, select = c(
#       sprintf('TABLE%s',ifelse(XwalkName=='FMstr','',toupper(XwalkDirection))),
#       sprintf('PARAMETER%s',toupper(XwalkDirection)))
#     )
#     names(XwalkParamCheck) = c('SAMPLE_TYPE','PARAMETER')
#     SPmissing = unique(
#       setdiff(
#         toupper( paste( TBLtmp$SAMPLE_TYPE, TBLtmp$PARAMETER, sep=':' ) ),
#         toupper( paste( XwalkParamCheck$SAMPLE_TYPE, XwalkParamCheck$PARAMETER, sep=':' ) )
#       )
#     )
#     if( length(SPmissing) > 0 ) {
#       cat('\nThe following parameters do not have a xwalk match:\n')
#       print.table(SPmissing)
#       cat('\n')
#     }
#   }
#   return(XwalkTBL)
