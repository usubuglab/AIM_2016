...
#! rename photos?
setwd('I:\\DCIM\\label')
photoextensionGREP='*.(?:jpe?g|gif|png)'#|mov
...
for (p in 1:length(photofiles)){
  ...
  prefix='OR-City-Newport-Emily-'#<Trip-Location><Owner>#Hanauma#will be tricky to separate some bob and em oregeon dscn (em's should all have parantheses if same number)...best bet is temporarily relocating them in batches (which is probably how to do them all)
